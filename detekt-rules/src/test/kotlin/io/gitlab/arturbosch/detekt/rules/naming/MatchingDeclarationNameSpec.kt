...
internal class MatchingDeclarationNameSpec : Spek({
    ...
    describe("MatchingDeclarationName rule") {
        ...
        context("compliant test cases") {
            ...
            it("should pass for data class by default") {
                val ktFile = compileContentForTest(
                    """
                        data class D(val i: Int)
                        fun D.doWithD() = Unit
                    """.trimMargin(),
                    filename = "DExtensions.kt"
                )
                val findings = MatchingDeclarationName().lint(ktFile)
                assertThat(findings).isEmpty()
            }
        ...
        context("non-compliant test cases") {
            ...
            it("should not pass for data class when ignoreDataClasses is false") {
                val ktFile = compileContentForTest(
                    """
                        data class D(val i: Int)
                        fun D.doWithD() = Unit
                ...
                val findings = MatchingDeclarationName(
                    TestConfig("ignoreDataClasses" to "false")
                ).lint(ktFile)
                assertThat(findings).hasSourceLocation(1, 25)
            }
        }
    }
})
